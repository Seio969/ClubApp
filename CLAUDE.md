...
- **Balance formula (`SaldoSocios.saldo_actual`), decided but not yet implemented**: `saldo_actual = saldo_anterior + cargos − pagos − reembolsos + devoluciones` (a `devolución` reverses the pago it bounced, so it adds back to what's owed — same direction as a `cargo`). It's a **continuous running total, not reset per período**: `saldo_anterior` chains forward at *every* período transition (each período's `saldo_anterior` = the prior período's `saldo_actual`), not just annually — the legacy `.xlsm`'s "Saldo Anterior" annual sheet was a one-time bootstrap value from before the app existed, not a recurring mechanic. No schema change needed for this — it fits the existing per-`(numero_socio, id_periodo)` `SaldoSocios` row shape (`database/models.py`), the rows just need to be populated with chained values rather than reset ones. `ReglaCobro.plazo_pago` = 10 days (recargo trigger), matching the legacy `.xlsm`'s "día 10" comment. `ReglaCobro.descuento` has no legacy precedent — leave unused/manual-only, don't build discount logic for it. See PLAN.md §2.15b for the `.xlsm` analysis this was decided from; the actual balance-calculation service (2.5) and period open/close logic (2.6) are still unbuilt.
