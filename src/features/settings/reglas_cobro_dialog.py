...
class ReglaCobroDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None, initial_data: Optional[dict[str, Any]] = None) -> None:
        ...
        self.cuota_mensual_input.setValidator(QDoubleValidator(0.0, 999999.99, 2, self))
        self.cuota_mensual_input.validator().setLocale(c_locale)
        ...
        self.penalizacion_input.setValidator(QDoubleValidator(0.0, 999999.99, 2, self))
        self.penalizacion_input.validator().setLocale(c_locale)
        self.descuento_input = QLineEdit(self)
        self.descuento_input.setValidator(QDoubleValidator(0.0, 999999.99, 2, self))
        self.descuento_input.validator().setLocale(c_locale)
