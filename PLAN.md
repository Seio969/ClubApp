...
5. **Live/incremental search (new request, 2026-08-09).** Today's search boxes only run a query on Enter or the "Buscar" button click (`MembersMenuView.on_search` wired to `search_input`'s submit + button; `TransactionsView.on_search` wired similarly to its "Buscar por socio…" input). User wants results to update as each character is typed instead — wire `search_input.textChanged` (in addition to/instead of the existing Enter/button triggers) to re-run the search on every keystroke. Consider debouncing (e.g. a short `QTimer` delay reset on each keystroke) so a fast typist doesn't fire a full DB query per character. Applies to both `features/members/menu_view.py` and `features/transactions/view.py`'s search inputs.
6. **Remove the "Límite" row-limit control (new request, 2026-08-09).** `MembersMenuView` has a "Límite:" numeric input (`self.limit_input`, `features/members/menu_view.py`) that caps how many rows `MembersMenuService.search_members` returns; user wants this functionality removed entirely — not just hidden. Means removing the input + its label from the top bar, `get_limit()`, the `on_limit_changed` handler, and the `limit` parameter/threading through `load_table_view()`/`search_members()` (decide whether `search_members` keeps an optional `limit` param unused by the UI or drops it outright — check no other caller relies on it first).
...
1. **Live/incremental search** (4.5) — new request, no open decisions, straightforward wiring change.
2. **Remove the "Límite" row-limit control** (4.6) — new request, no open decisions, straightforward removal.
3. **Table selection persistence** (4.4) — still unresolved; see §4 item 4's note on two rejected attempts before trying again.
4. **Estado-aware filtering for deactivated members** (2.2) — decide and implement how inactive members are handled on reload (today they silently reappear, just correctly marked `inactivo`). A user question on the exact default (hide vs. show-distinguished vs. Filtros-menu option) went unanswered — ask again before implementing.
