...
**Done** (branch `feat/seed-data-and-settings`): `metodos_pago` seeding (the 5 fixed README methods, idempotent, run from `init_db()`); `⚙️ Ajustes` now opens a real Settings hub (`features/settings/menu_view.py`) with section buttons rather than being a no-op. Both hub sections have full CRUD: "💳 Métodos de pago" (`metodos_pago_view.py`) — add/rename/activate-deactivate custom methods, the 5 fixed methods can be deactivated but never renamed/deleted; "📋 Reglas de cobro" (`reglas_cobro_view.py`) — the club can define multiple named rules (e.g. "Cuota general", "Cuota reducida"), each with cuota_mensual/plazo_pago/penalizacion/descuento, fully editable. Both sections never physically delete a row — "remove" is soft-deactivation via an `estado` column (same pattern as `Socio`), added to both `MetodoPago` and `ReglaCobro`.
