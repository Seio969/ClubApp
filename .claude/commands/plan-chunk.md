...
2. Before writing any code, create a feature branch for this chunk (use `/branch`) and publish it to origin right away (`/push`, no PR yet) — `main` is protected on GitHub, no direct pushes. Commit (`/commit`) and push as you go so the remote branch stays current with progress.
...
4. Once everything in this chunk is done and approved, update PLAN.md and CLAUDE.md so PLAN.md stays lean for the next session to read in full — do this *before* drafting the PR, since both files land on this same branch and the PR description should describe the branch's final contents:
   ...
   - Commit and push both (all) updated docs.
5. As the final step — after docs are committed and pushed, not before — give a draft: a suggested PR title, and a body with a "## Summary" (bullet points of what changed and why, including the doc updates alongside the code) and a "## Test plan" section with concrete, numbered manual-test steps (not just a checkbox), formatted the same way this repo's existing PRs are written. Do not open the PR — the `gh` CLI isn't installed here (see CLAUDE.md); the user opens it manually. Then STOP — don't continue to any other PLAN.md section.
